...
func newBalanceHotReadRegionsScheduler(opt *scheduleOption) *balanceHotReadRegionsScheduler {
...
func (h *balanceHotReadRegionsScheduler) GetResourceKind() ResourceKind {
	return PriorityKind
}
...
func (h *balanceHotReadRegionsScheduler) Prepare(cluster *clusterInfo) error { return nil }

func (h *balanceHotReadRegionsScheduler) Cleanup(cluster *clusterInfo) {}

func (h *balanceHotReadRegionsScheduler) Schedule(cluster *clusterInfo) Operator {
	schedulerCounter.WithLabelValues(h.GetName(), "schedule").Inc()
	h.calcScore(cluster)
...
func (h *balanceHotReadRegionsScheduler) calcScore(cluster *clusterInfo) {
	h.Lock()
	defer h.Unlock()
...
func (h *balanceHotReadRegionsScheduler) balanceByPeer(cluster *clusterInfo) (*RegionInfo, *metapb.Peer, *metapb.Peer) {
	var (
		maxReadBytes           uint64
		srcStoreID             uint64
		maxHotStoreRegionCount int
...
func (h *balanceHotReadRegionsScheduler) selectDestStoreByPeer(candidateStoreIDs []uint64, srcRegion *RegionInfo, srcStoreID uint64) uint64 {
	sr := h.statisticsAsPeer[srcStoreID]
...
func (h *balanceHotReadRegionsScheduler) balanceByLeader(cluster *clusterInfo) (*RegionInfo, *metapb.Peer) {
	var (
		maxReadBytes           uint64
		srcStoreID             uint64
		maxHotStoreRegionCount int
...
func (h *balanceHotReadRegionsScheduler) selectDestStoreByLeader(srcRegion *RegionInfo) *metapb.Peer {
	sr := h.statisticsAsLeader[srcRegion.Leader.GetStoreId()]
	srcReadBytes := sr.ReadBytes
	srcHotRegionsCount := sr.ReadRegionsStat.Len()
